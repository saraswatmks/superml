...
#' @description Trains a Extreme Gradient Boosting Model. It is a wrapper around xgboost R package to provide a similar interface to scikit-learn. You simply need to provide a data-frame or data.table to make it work.
...
#'  \item{print_every}{print training log after n interations. Default = 50}
...
#'  \item{custom_objective}{custom objective functioon}
...
#' xgb <- XGBTrainer$new(objective = 'multi:softmax', maximize = F, eval_metric = 'merror', num_class=3, n_estimators = 500)
#' xgb$fit(df, 'Species')
